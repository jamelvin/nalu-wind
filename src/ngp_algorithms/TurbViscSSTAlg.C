...
TurbViscSSTAlg::execute()
{
  ...
  nalu_ngp::run_entity_algorithm(
    ...
    KOKKOS_LAMBDA(const Traits::MeshIndex& meshIdx) {
      ...
      //      if ((coords.get(meshIdx,0) <= 2.0) && (coords.get(meshIdx,1)
      //      <= 1.0)) {
      //          NaluEnv::self().naluOutput() << coords.get(meshIdx, 0) << " "
      //          << coords.get(meshIdx, 1) << " " << coords.get(meshIdx, 2) <<
      //          " " << tvisc.get(meshIdx, 0) << " " << sdr.get(meshIdx, 0) <<
      //          " " << tke.get(meshIdx, 0) << " " << aOne*sdr.get(meshIdx, 0)
      //          << " " << sijMag*fTwo << " " <<
      //          stk::math::max(aOne*sdr.get(meshIdx, 0), sijMag*fTwo) <<
      //          std::endl;
      //      }
