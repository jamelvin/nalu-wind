    ...
    nDim_(bulk.mesh_meta_data().spatial_dimension())
{
  ...
  edgeAreaVecID_ = get_field_ordinal(meta, "edge_area_vector", stk::topology::EDGE_RANK);
  ...
  for (int i = 0; i < ndim; ++i) {
    ...
    for (int l = 0; l < ndim; ++l) {
      ...
      const EdgeKernelTraits::DblType GlfluctUi =
        0.5 * (dudx_.get(nodeL, offSetIL) + dudx_.get(nodeR, offSetIL)) - GlavgUi;
    ...
    for (int j = 0; j < ndim; ++j) {
      ...
      const EdgeKernelTraits::DblType GjfluctUi =
        0.5 * (dudx_.get(nodeL, offSetIJ) + dudx_.get(nodeR, offSetIJ)) - GjavgUi;
      avgdUidxj[i][j] =
