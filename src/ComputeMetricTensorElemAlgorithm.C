...
//--------------------------------------------------------------------------
void
ComputeMetricTensorElemAlgorithm::execute()
{
  ...
  for ( stk::mesh::BucketVector::const_iterator ib = elem_buckets.begin();
    ...
    for (stk::mesh::Bucket::size_type k = 0; k < length; ++k) {
      ...
      // compute geometry and get Mij from master element function at integration points
      double scv_error = 0.0;
      meSCV->grad_op(1, &p_coords[0], &ws_dndx[0], &ws_deriv[0], &ws_det_j[0], &scv_error);
      meSCV->Mij(&p_coords[0], &p_Mij[0], &ws_deriv[0]);
      
      // since we only want a single elemental value, average over all integration points
      for (int ip = 0; ip < numScvIp; ++ip)
        for (int i = 0; i < nDim; ++i)
          for (int j = 0; j < nDim; ++j)
              Mij[i*nDim + j] += p_Mij[ip*nDim*nDim + i*nDim + j] / numScvIp;
    }
  }
